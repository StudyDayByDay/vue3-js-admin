<template>
  <div class="clickMenu" v-loading="loading">
    <div class="clickMenu-header">
      ...
      <el-button
        :type="carverBtn ? 'danger' : 'primary'"
        plain
        @click="handleTransfer"
        >{{
          `${carverBtn ? "结束" : "开始"}${scribeCopy ? "复制" : "划词"}`
        }}</el-button
      >
      <el-button
        :type="connectBtn ? 'danger' : 'primary'"
        plain
        @click="handleConnect"
        >{{ `${connectBtn ? "结束" : "开始"}连线` }}</el-button
      >
    </div>
...
import {
  Carver,
  globalOffsetToPageOffset,
  pageOffsetToGlobalOffset,
} from "@/utils";
...
const handleScroll = () => {
  ...
  const mutationObserver = new MutationObserver(() => {
    ...
    // 之前最靠近边缘元素当前的偏移等
    var currentTopPOffset =
      Math.round(carverPanel.value.getBoundingClientRect().top) -
      Math.round(targetEle.getBoundingClientRect().top);
    // 滚动修正
...
const copyCommit = async () => {
  const {
    data: { entityRelationVoList, mrEntityVoList },
  } = await apis.copy(copyParams);
  // 进行渲染
...
const handleCopy = (scribble) => {
  ...
  switch (entitysInTemplate.length) {
    ...
    case 2:
      ...
      if (
        start > -1 &&
        parseInt(entitysInTemplate[0].endOffset) + 1 ==
          entitysInTemplate[1].startOffset
      ) {
        const entity1 = {
          id: Math.floor(Math.random() * (99999999 - 999999 + 1)) + 999999,
          ...
          endOffset:
            parseInt(scribbleStart) +
            start +
            entitysInTemplate[0].text.length -
            1,
          labelId: entitysInTemplate[0].labels[0].id,
          labelText: entitysInTemplate[0].labels[0].title,
          mrId,
        ...
        const entity2Text = scribbleText.substr(
          entitysInTemplate[0].text.length
        );
        const entity2 = {
          id: Math.floor(Math.random() * (99999999 - 999999 + 1)) + 999999,
          text: entity2Text,
          startOffset:
            parseInt(scribbleStart) + start + entitysInTemplate[0].text.length,
          endOffset:
            parseInt(scribbleStart) +
            start +
            entitysInTemplate[0].text.length +
      ...
      if (newEntitys.length == 2) {
        ...
        if (hasRelation) {
          ...
          for (let i = 0; i < entityRelations.length; i++) {
            ...
            //关系有正向、反向
            if (
              entityRelation.fromId == entitysInTemplate[0].id &&
              entityRelation.toId == entitysInTemplate[1].id
            ) {
            ...
            if (
              entityRelation.fromId == entitysInTemplate[1].id &&
              entityRelation.toId == entitysInTemplate[0].id
            ) {
              reationId = entityRelation.relationId;
              newEntityRelations.push({
                from: 1,
    ...
    default:
          ...
          if (/^[\u4e00-\u9fa5]+$/.test(entity.text)) {
            ...
            if (result && result.length > 0) {
              ...
              entitysInScribble.push({
                ...
                endOffset:
                  scribbleStart + offset + start + result[0].length - 1,
      ...
      for (let i = 0; !wrong && i < seperators.length; i++) {
        ...
        //以分隔符去寻找实体
        const text = scribbleText.substr(
          seperatorOffset,
          index - seperatorOffset
        );
        newEntitys.push({
          id: Math.floor(Math.random() * (99999999 - 999999 + 1)) + 999999,
          text: text,
          ...
          endOffset:
            parseInt(scribbleStart) + seperatorOffset + text.length - 1,
      ...
      if (!wrong) {
        ...
        newEntitys.push({
          ...
          startOffset: parseInt(scribbleStart) + seperatorOffset,
          endOffset:
            parseInt(scribbleStart) + seperatorOffset + last.length - 1,
          labelId: entitysInTemplate[entitysInTemplate.length - 1].labels[0].id,
          labelText:
            entitysInTemplate[entitysInTemplate.length - 1].labels[0].title,
          mrId,
        });
      } else {
  ...
  copyParams.mrEntityRelationRequestList = newEntityRelations.map(
    (newEntityRelation) => {
      const { text: startText, labelText: startLabel } =
          newEntitys[newEntityRelation.from],
        { text: endText, labelText: endLabel } =
          newEntitys[newEntityRelation.to];
      return {
        startText,
        startLabel,
...
const carverSelect = ({
  ...
  if (scribeCopy.value) {
    ...
    // 如果划取的内容里面有实体就展示复制按钮
    scribeProps.copyShow = entitys.some(
      (entity) =>
        !(entity.startOffset > scribbleEnd || entity.endOffset < scribbleStart)
    );
    if (scribeProps.copyShow) {
      // 初始化复制模版
      const entitysInTemplate = entitys.filter(
        (entity) =>
          !(
            entity.startOffset > scribbleEnd || entity.endOffset < scribbleStart
          )
      ...
      // 取出包含划词中实体的真实文本内容
      const scribbleText = page.text.substr(
        globalOffsetToPageOffset(start, page),
        end - start + 1
      );
...
const renderPageModule = () => {
  return new Promise((resolve, reject) => {
    ...
    // 过滤在当前文章内的模块
    const marks = moduleData
      .filter((item) => {
        return (
          item.startOffset >= contentStartOffset &&
          item.endOffset <= contentEndOffset
        );
      })
      .map((item) => {
        return {
...
const renderPageEntitys = (alone) => {
  return new Promise((resolve, reject) => {
    ...
    const marks = handleArr
      .filter((entity) => {
        return (
          entity.startOffset >= contentStartOffset &&
          entity.endOffset <= contentEndOffset
        );
      })
      .map((item) => {
        return {
...
const renderPageEntityRelation = (alone) => {
  return new Promise((reslove, reject) => {
    ...
    handleArr
      .filter((item) => {
        return (
          item.from.startOffset >= contentStartOffset &&
          item.from.endOffset <= contentEndOffset
        );
      })
      .forEach((item) => {
        promiseArr.push(
          renderEntityRelation(
            item.id,
            item.fromId,
            item.toId,
            item.relationVo.title
          )
        );
      });
...
const scribeTransferChange = (e) => {
  if (scribeProps.type === "label") {
    ...
    if (connectBtn.value) {
      const {
        labels: [{ id: relationId }],
      } = e;
      carver
        .connect(scribeProps.target)
